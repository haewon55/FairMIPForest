...
For each dataset, we provide 1) an example notebook where you can train a new Fair MIP Forest model with different parameters, 2) a notebook that opens already-trained Fair MIP Forest models stored in the sub-folder 'forests', and 3) notebooks where you can run other benchmarks (Equalized Odds, Disparate Mistreatement, and Exponentiated Gradient) on the dataset with missing values.
